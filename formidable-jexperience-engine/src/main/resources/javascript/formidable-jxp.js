...
(function () {
  ...
  if (required.length > 0 && Array.isArray(window.digitalDataOverrides)) {
  ...
  /**
   * Fills the mapped fields of the form from the loaded context, one field at a time, and never twice:
   * a value the visitor already typed is left alone, and a default value the author gave the field
   * gives way only when the author said so (`overridesDefault`). Nothing happens without a tracker, a
   * profile and a hydrated form.
   */
  api.prefill = (formId) => {
    ...
    Object.keys(pairs).forEach((field) => {
      const value = values[pairs[field].property];
      if (value !== undefined && value !== null && value !== "") {
        fill(form, field, value, Boolean(pairs[field].overridesDefault));
      }
    });
    return true;
  };
  ...
  /**
   * Writes one profile value into the controls named `field`, by their shape. `input` then `change`
   * are dispatched on what changed, so the conditional logic, the input mask and any validation see
   * the value as if the visitor had typed it.
   */
  function fill(form, field, value, overridesDefault) {
    ...
    if (type === "checkbox" || type === "radio") {
      fillChoices(controls, value, overridesDefault);
    } else if (first.tagName === "SELECT") {
      fillSelect(first, value, overridesDefault);
    } else if (type !== "file" && type !== "submit" && type !== "button" && type !== "reset") {
      fillText(first, value, overridesDefault);
    }
  }

  /** text, email, tel, url, number, date, datetime-local, hidden, textarea: one string. */
  function fillText(control, value, overridesDefault) {
    if (control.value !== control.defaultValue) {
      return; // the visitor typed
    }
    if (control.defaultValue !== "" && !overridesDefault) {
      return; // the author's default stays
    }
  ...
  function fillChoices(controls, value, overridesDefault) {
    if (controls.some((c) => c.checked !== c.defaultChecked)) {
      return; // the visitor chose
    }
    if (controls.some((c) => c.defaultChecked) && !overridesDefault) {
      return; // the author's default stays
    }
    ...
    controls.forEach((control) => {
      ...
      const check = single ? value === true || value === "true" || wanted.indexOf(control.value) > -1
  ...
  function fillSelect(select, value, overridesDefault) {
    ...
    if (options.some((o) => o.selected !== o.defaultSelected)) {
      return; // the visitor chose
    }
    if (options.some((o) => o.defaultSelected && o.value !== "") && !overridesDefault) {
      return; // the author's default stays
    }
    const wanted = Array.isArray(value) ? value.map(String) : [String(value)];
    ...
    if (select.multiple) {
      options.forEach((o) => { o.selected = matching.indexOf(o) > -1; });
    } else {
