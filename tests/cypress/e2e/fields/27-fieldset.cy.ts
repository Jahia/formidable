...
describe("Form fields - 27 Fieldset", () => {
  ...
  it("groups several fields under the same fieldset in live mode", () => {
    createPublishedLiveFormPage("fieldset-form", "Fieldset Form", [
      getFieldsetNode({
        ...FIELDSET_PROFILE,
        children: [
      ...
      fieldset.shouldBeVisible().shouldHaveLegend(FIELDSET_PROFILE.title!);

      fieldset.getTextInput(INPUT_TEXT_COMPLETE.name!).shouldBeVisible().shouldHaveValue("AB-1234");
