...
const getPropertyValue = (
  properties: JcrPropertyResponse[] | undefined,
  propertyName: string,
): string | undefined =>
      ...
      getLatestLiveFormSubmission(formName)
        ...
        .then((response: NodeByPathResponse) => {
          ...
          savedFiles.forEach((savedFile) => {
            const expectedMimeTypes = FIXTURE_FILES.find(
              (file) => file.fileName === savedFile.fileName,
            )?.mimeTypes;
            ...
            expect(savedFile.mimeType, `saved MIME type for ${savedFile.fileName}`).to.be.oneOf(
              expectedMimeTypes,
            );
          });
        });
