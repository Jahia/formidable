import { getRadioNode, RADIO_GROUP, RADIO_SINGLE } from "../../support/fixtures";
...
describe("Form fields - 23 Radio", () => {
  ...
  it("renders a required standalone radio option", () => {
    createPublishedLiveFormPage("radio-single-form", "Radio Single Form", [
      getRadioNode(RADIO_SINGLE),
    ]).then(({ livePath }) => {
      const form = visitLiveForm(livePath);

      form
        .getRadio(RADIO_SINGLE.name!)
        .shouldBeVisible()
        .shouldBeRequired()
  ...
  it("renders a required radio group and switches selection correctly", () => {
    createPublishedLiveFormPage("radio-group-form", "Radio Group Form", [
