...
describe('Form fields - 23 Radio', () => {
	...
	it('renders a required standalone radio option', () => {
		createFormNode('radio-single-form', 'Radio Single Form', [getRadioNode(RADIO_SINGLE)]);

		const form = getFormPreview('Radio Single Form');
	...
	it('renders a required radio group and switches selection correctly', () => {
		createFormNode('radio-group-form', 'Radio Group Form', [getRadioNode(RADIO_GROUP)]);

		const form = getFormPreview('Radio Group Form');
