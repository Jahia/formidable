...
describe("Form fields - 20 All field types", () => {
  ...
  it("submits a simple live form with all supported field types", () => {
    createPublishedLiveFormPage("all-fields-simple-form", "All Fields Simple Form", [
      ...
      form.getTextInput(INPUT_TEXT_COMPLETE.name!).type("AB-9876").shouldBeValid();
      form.getEmailInput(INPUT_EMAIL_COMPLETE.name!).type("contact@example.com").shouldBeValid();
      form.getDateInput(INPUT_DATE_COMPLETE.name!).setDate("2001-02-03").shouldBeValid();
      form
        .getDateTimeLocalInput(INPUT_DATETIME_LOCAL_COMPLETE.name!)
        .setDateTime("2026-06-12T09:30")
        .shouldBeValid();
      form.getColorInput(INPUT_COLOR_COMPLETE.name!).setColor("#336699").shouldBeValid();
      form.getCheckboxGroup(CHECKBOX_GROUP_COMPLETE.name!).checkByLabels(["Sports"]);
      form.getRadioGroup(RADIO_GROUP.name!).select("Pickup").shouldHaveSelected("Pickup");
      form
        .getSelectInput(SELECT_SINGLE.name!)
        .shouldHaveSelectedOption("Please select")
        .select("Support")
        .shouldHaveSelectedOption("Support");
      form
        .getTextarea(TEXTAREA_COMPLETE.name!)
        .type("A longer project summary that satisfies the field constraints.")
        .shouldBeValid();
      form
        .getFileInput(INPUT_FILE_MULTIPLE.name!)
  ...
  it("submits a multistep live form with all supported field types distributed across steps", () => {
    createPublishedLiveFormPage("all-fields-step-form", "All Fields Step Form", [
      getStepNode({
        name: "identityStep",
        title: "Identity",
        label: "Identity",
      ...
      form
        .getDateTimeLocalInput(INPUT_DATETIME_LOCAL_COMPLETE.name!)
        .setDateTime("2026-06-20T14:15");
      form.getColorInput(INPUT_COLOR_COMPLETE.name!).setColor("#663399");
      form
        .getTextarea(TEXTAREA_COMPLETE.name!)
        .type("Second step details long enough for textarea validation.");
      form
        .getFileInput(INPUT_FILE_MULTIPLE.name!)
