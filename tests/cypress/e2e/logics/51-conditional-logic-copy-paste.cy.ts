...
const getNodeUuidByPath = (path: string): Cypress.Chainable<string> =>
  getNodeByPath(path).then((response: NodeByPathResponse) => {
...
describe("Form logic - 51 Conditional logic copy and paste", () => {
  ...
  it("copies a whole form and rebinds the weakref inside the copied form", () => {
    createConditionalLogicForm(`${Date.now()}-full-copy`).then(
      ({ formName, formPath, rolePath, targetPath }) => {
        storeLogicRule(
          targetPath,
          rolePath,
          "role",
        ...
        getConditionalLogicNode(copiedTargetPath).then((node) => {
          const rawLogics =
            node.properties?.find((property) => property.name === "logics")?.values ?? [];
          ...
          expect(storedRule.sourceNodeId).to.equal(
            node.descendant?.children?.nodes?.[0]?.property?.refNode?.uuid,
          );
  ...
  it("copies a whole form with duplicate source names and keeps the correct source binding", () => {
    createDuplicateSourceNameConditionalLogicForm(`${Date.now()}-duplicates`).then(
      ({ formName, formPath, secondSourcePath, targetPath }) => {
        storeLogicRule(
          targetPath,
          secondSourcePath,
          "select-an-option",
        ...
        getNodeByPath(copiedSecondSourcePath, ["options"], "en").then(
          (response: NodeByPathResponse) => {
            const optionValues =
              response.data?.jcr?.nodeByPath?.properties?.find(
  ...
  it("copies a single logic field without a matching local source and keeps only the JSON rule", () => {
    createConditionalLogicForm(`${Date.now()}-source-field`).then(
      ({ rolePath, targetPath: sourceTargetPath }) => {
        storeLogicRule(
          sourceTargetPath,
          rolePath,
          "role",
          ...
          "in",
          ["admin"],
          "logic-single-field-copy",
        );
        waitForReferencedSourcePath(sourceTargetPath, rolePath);

        createConditionalLogicNoMatchingSourceForm(`${Date.now()}-target-form`).then(
          ({ formPath }) => {
            ...
            cy.waitUntil(() =>
              findConditionalLogicNode(copiedTargetPath).then((node) => {
                if (!node) {
                  return false;
                }
