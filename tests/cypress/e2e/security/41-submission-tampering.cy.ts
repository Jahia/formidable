...
describe("Security - submission tampering", () => {
  ...
  it("ignores an undeclared text field in the submitted multipart body", () => {
    ...
    postDirectMultipartSubmission({
      ...
      cy.login();
      getLatestLiveFormSubmission("tampering-form").then(({ path }) => {
        // Read the data child node to inspect stored field values.
        getNodeByPath(`${path}/data`, ["fullName", "role"], "", [], "LIVE").then(
          (dataResponse: NodeByPathResponse) => {
  ...
  it("ignores an undeclared file part in the submitted multipart body", () => {
    ...
    postDirectMultipartSubmission({
      ...
      getLatestLiveFormSubmission("tampering-form").then(({ path }) => {
        ...
        getNodeByPath(`${path}/data`, ["fullName", "maliciousFile"], "", [], "LIVE").then(
          (dataResponse: NodeByPathResponse) => {
            const properties = dataResponse.data?.jcr?.nodeByPath?.properties ?? [];
            ...
            expect(
              maliciousFileProp,
              'undeclared field "maliciousFile" should not be stored in data',
            ).to.not.exist;
          },
        );

        // Verify no file metadata or binary is stored for the undeclared file field.
        getNodeByPath(path, [], "", ["jnt:folder"], "LIVE").then(
          (submissionResponse: NodeByPathResponse) => {
            ...
            if (filesFolder) {
              ...
              expect(maliciousFolder, "undeclared file field should not have a storage folder").to
                .not.exist;
            }
          },
        );
