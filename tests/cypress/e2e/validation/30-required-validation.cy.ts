...
describe("Validation - 30 Required validation", () => {
  ...
  it("shows a custom required message for a select field and clears it after selection", () => {
    createPublishedLiveFormPage(
      ...
      "Validation Required Select Form",
      [
        withValidationMessages(getSelectNode(REQUIRED_SELECT), {
  ...
  it("shows a custom required message for a checkbox and clears it after checking", () => {
    createPublishedLiveFormPage(
      ...
      "Validation Required Checkbox Form",
      [
        withValidationMessages(getCheckboxNode(REQUIRED_CHECKBOX), {
      ...
      checkbox.shouldHaveValidationError(CHECKBOX_MESSAGE).shouldBeMarkedInvalid();

      checkbox.check().shouldBeValid().shouldNotHaveValidationError().shouldNotBeMarkedInvalid();
    });
  });
  ...
  it("shows a custom required message for a checkbox group and clears it after checking one option", () => {
    createPublishedLiveFormPage(
      ...
      "Validation Required Checkbox Group Form",
      [
        withValidationMessages(getCheckboxNode(REQUIRED_CHECKBOX_GROUP), {
      ...
      checkbox.shouldHaveValidationError(CHECKBOX_GROUP_MESSAGE).shouldBeMarkedInvalid();

      checkboxGroup.getCheckbox("News").check().shouldBeChecked();

      checkbox.shouldBeValid().shouldNotHaveValidationError().shouldNotBeMarkedInvalid();
    });
  });

  it("shows a custom required message for a file input when no file is selected", () => {
    createPublishedLiveFormPage("validation-required-file-form", "Validation Required File Form", [
      withValidationMessages(getInputFileNode(REQUIRED_FILE), { msgValueMissing: FILE_MESSAGE }),
    ]).then(({ livePath }) => {
      const form = visitLiveForm(livePath);
      ...
      fileInput.shouldHaveValidationError(FILE_MESSAGE).shouldBeMarkedInvalid();
    });
  });
  ...
  it("shows a custom required message for a radio group and clears it after selection", () => {
    createPublishedLiveFormPage(
      ...
      "Validation Required Radio Form",
      [withValidationMessages(getRadioNode(REQUIRED_RADIO), { msgValueMissing: RADIO_MESSAGE })],
    ).then(({ livePath }) => {
      const form = visitLiveForm(livePath);
      ...
      radio.shouldHaveValidationError(RADIO_MESSAGE).shouldBeMarkedInvalid().shouldBeFocused();

      radioGroup.select("Email").shouldHaveSelected("Email");

      radio.shouldBeValid().shouldNotHaveValidationError().shouldNotBeMarkedInvalid();
    });
  });
});
