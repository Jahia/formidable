...
describe("Validation - 31 Textual validation", () => {
  ...
  it("shows custom messages for required, pattern, and tooShort text constraints", () => {
    createPublishedLiveFormPage("validation-textual-form", "Validation Textual Form", [
      ...
      textInput.shouldHaveValidationError(TEXT_MESSAGES.msgValueMissing).shouldBeMarkedInvalid();

      textInput.type("INVALID");
      form.submit();

      textInput.shouldHaveValidationError(TEXT_MESSAGES.msgPatternMismatch).shouldBeMarkedInvalid();
      ...
      textarea.shouldHaveValidationError(TEXTAREA_MESSAGE).shouldBeMarkedInvalid();
  ...
  it("shows a custom tooShort message for a text input with a minLength constraint", () => {
    createPublishedLiveFormPage(
      ...
      "Validation Text MinLength Form",
      [
        withValidationMessages(getInputTextNode(MIN_LENGTH_TEXT_FIELD), {
      ...
      textInput.shouldHaveValidationError(MIN_LENGTH_TEXT_MESSAGE).shouldBeMarkedInvalid();
  ...
  it("falls back to the native browser message when no email override is configured", () => {
    createPublishedLiveFormPage("validation-native-email-form", "Validation Native Email Form", [
      ...
      emailInput.getInput().then(($input) => {
        ...
        emailInput.shouldHaveValidationError(input.validationMessage).shouldBeMarkedInvalid();
      });
    });
  });

  it("shows a custom typeMismatch message for an email field when configured", () => {
    createPublishedLiveFormPage("validation-custom-email-form", "Validation Custom Email Form", [
      withValidationMessages(getInputEmailNode(CUSTOM_EMAIL_FIELD), {
      ...
      emailInput.shouldHaveValidationError(CUSTOM_TYPE_MISMATCH_MESSAGE).shouldBeMarkedInvalid();
