...
describe("Validation - 33 Validation flow", () => {
  ...
  it("clears inline validation errors and invalid styling on form reset", () => {
    createPublishedLiveFormPage(
      ...
      textarea.shouldHaveValidationError(RESET_MESSAGE).shouldBeMarkedInvalid();

      form.reset();

      textarea.shouldNotHaveValidationError().shouldNotBeMarkedInvalid();
    });
  });

  it("focuses the first invalid field, then moves focus to the next invalid field after correction", () => {
    createPublishedLiveFormPage("validation-focus-form", "Validation Focus Form", [
      withValidationMessages(getInputTextNode(FIRST_FIELD), { msgValueMissing: FIRST_MESSAGE }),
      withValidationMessages(getInputEmailNode(SECOND_FIELD), { msgValueMissing: SECOND_MESSAGE }),
    ]).then(({ livePath }) => {
      const form = visitLiveForm(livePath);
      ...
      firstInput.shouldHaveValidationError(FIRST_MESSAGE).shouldBeMarkedInvalid().shouldBeFocused();

      secondInput.shouldHaveValidationError(SECOND_MESSAGE).shouldBeMarkedInvalid();
      ...
      secondInput.shouldHaveValidationError(SECOND_MESSAGE).shouldBeFocused();

      firstInput.shouldNotHaveValidationError().shouldNotBeMarkedInvalid();
    });
  });
});
