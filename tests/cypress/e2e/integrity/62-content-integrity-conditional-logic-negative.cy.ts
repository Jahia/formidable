...
import { createConditionalLogicForm, findConditionalLogicNode } from "../../support/fixtures";
...
describe("Content integrity - 62 Conditional logic negative detection", () => {
  ...
  it("detects a missing logicsSrc child for an existing conditional-logic rule", () => {
    ...
    createConditionalLogicForm(`${Date.now()}-missing-logicsrc`).then(
      ({ formPath, rolePath, targetPath }) => {
        ...
        runContentIntegrityScan({
          ...
          const matchingError = results.errors.find(
            (error) =>
