...
const IMPORT_CONTENT = gql`
  mutation importConditionalLogicFixture(
    $file: String!
...
export const createConditionalLogicForm = (
  suffix: string,
): Cypress.Chainable<ConditionalLogicFormInfo> => {
...
export const createDuplicateSourceNameConditionalLogicForm = (
  ...
  return createFormNode(
    formName,
    formName,
    DUPLICATE_SOURCE_NAME_CONDITIONAL_LOGIC_FORM_ELEMENTS,
  ).then(() => ({
    formName,
    formPath: `${CONTENT_PATH}/${formName}`,
...
export const createConditionalLogicNoMatchingSourceForm = (
  ...
  return createFormNode(
    formName,
    formName,
    CONDITIONAL_LOGIC_NO_MATCHING_SOURCE_FORM_ELEMENTS,
  ).then(() => ({
    formName,
    formPath: `${CONTENT_PATH}/${formName}`,
    targetPath: `${CONTENT_PATH}/${formName}/fields/nickname`,
...
export const findConditionalLogicNode = (
  targetPath: string,
): Cypress.Chainable<ConditionalLogicNode | null> =>
  cy
    .apollo({
    ...
    .then(
      (response: { data?: { jcr?: { nodeByPath?: ConditionalLogicNode | null } | null } }) =>
        response.data?.jcr?.nodeByPath ?? null,
    );

export const getConditionalLogicNode = (
  targetPath: string,
): Cypress.Chainable<ConditionalLogicNode> =>
  findConditionalLogicNode(targetPath).then((node) => {
    if (!node) {
...
export const parseStoredLogicRule = (rawValue: string): ConditionalLogicRule =>
  JSON.parse(rawValue) as ConditionalLogicRule;
...
const importXmlContentFixture = (
  ...
  cy.fixture(fixturePath, "utf8").then((rawContent) => {
...
export const importConditionalLogicForm = (
  suffix: string,
): Cypress.Chainable<ImportedConditionalLogicFormInfo> => {
  const formName = `conditional-logic-import-${suffix}`;

  return importXmlContentFixture("imports/conditional-logic-form.xml", `${formName}.xml`, {
...
export const importDuplicateSourceNameConditionalLogicForm = (
  ...
  return importXmlContentFixture("imports/conditional-logic-duplicates.xml", `${formName}.xml`, {
