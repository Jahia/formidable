/** Common types for Jahia node creation in Cypress tests Based on CND definitions in /src/components */
...
/**
 * Base interface for form element data All form elements extend fmdb:element which extends
 * mix:title
 */
...
/**
 * Base interface for input elements with validation Based on common properties found in CND
 * definitions
 */
...
/**
 * Checkbox data based on fmdb:checkbox CND 1 choice → standalone checkbox, N choices → checkbox
 * group
 */
...
/** Radio data based on fmdb:radio CND 1 choice → standalone radio, N choices → radio group */
...
/**
 * Color input data based on fmdb:inputColor CND Note: Currently only has required and defaultValue
 * Future: alpha, colorspace for advanced settings
 */
export type InputColorData = InputWithDefaultValue;

/** Date input data based on fmdb:inputDate CND Inherits: required, defaultValue, min, max, step */
export type InputDateData = InputWithRange;

/**
 * Datetime-local input data based on fmdb:inputDatetimeLocal CND Inherits: required, defaultValue,
 * min, max, step
 */
