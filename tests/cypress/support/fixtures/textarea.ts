...
export function getTextareaNode(data: TextareaData = TEXTAREA_COMPLETE): JahiaNode {
  ...
  if (data.placeholder)
    properties.push({ name: "placeholder", value: data.placeholder, language: "en" });
  if (data.defaultValue)
    properties.push({ name: "defaultValue", value: data.defaultValue, language: "en" });
  if (data.required !== undefined)
    properties.push({ name: "required", value: String(data.required), type: "BOOLEAN" });
  if (data.minLength !== undefined)
    properties.push({ name: "minLength", value: String(data.minLength), type: "LONG" });
  if (data.maxLength !== undefined)
    properties.push({ name: "maxLength", value: String(data.maxLength), type: "LONG" });
  if (data.rows !== undefined)
    properties.push({ name: "rows", value: String(data.rows), type: "LONG" });
  if (data.cols !== undefined)
    properties.push({ name: "cols", value: String(data.cols), type: "LONG" });
  if (data.autocomplete) properties.push({ name: "autocomplete", value: data.autocomplete });
  if (data.spellcheck !== undefined)
    properties.push({ name: "spellcheck", value: String(data.spellcheck), type: "BOOLEAN" });
  if (data.readonly !== undefined)
    properties.push({ name: "readonly", value: String(data.readonly), type: "BOOLEAN" });
  if (data.autofocus !== undefined)
    properties.push({ name: "autofocus", value: String(data.autofocus), type: "BOOLEAN" });
  if (data.disabled !== undefined)
    properties.push({ name: "disabled", value: String(data.disabled), type: "BOOLEAN" });
