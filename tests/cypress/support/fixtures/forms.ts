...
export const createPublishedLiveFormPage = (
  ...
  return createFormNode(formName, formTitle, formElements, options)
    .then((response: AddNodeResponse) => {
      ...
      if (!formId) {
        throw new Error(
          `Could not resolve UUID for form '${formName}': ${JSON.stringify(response)}`,
        );
      }
        ...
        children: [
          {
            ...
            children: [
              ...
              },
              {
                ...
                properties: [{ name: "j:node", value: formId, type: "WEAKREFERENCE" }],
              },
            ],
          },
        ],
...
export function getFormPreview(formTitle: string): Form {
  ...
  return new Form(
    cy
      .get(JCONTENT_SELECTORS.previewIframe)
...
export function visitLiveForm(livePath: string, lang: string = "en"): Form {
  ...
  return new Form(cy.get("form.fmdb-form").should("exist").first());
}

/**
 * Opens a form page in preview (default workspace, render servlet) and returns the Form page
 * object. Requires an authenticated session (cy.login()).
 */
...
export const getLatestLiveFormSubmission = (
  formName: string,
): Cypress.Chainable<LiveFormSubmissionInfo> => {
  const submissionsRootPath = `/sites/${FORMIDABLE_TEST_SITE.key}/formidable-results/${formName}/submissions`;

  return cy
    .request<LatestSubmissionQueryResponse>({
      method: "POST",
      url: "/modules/graphql",
      headers: {
    ...
    .then((response) => {
      ...
      const submission = submissions.sort(
        (left, right) =>
