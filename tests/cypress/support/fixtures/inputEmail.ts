...
export function getInputEmailNode(data: InputEmailData = INPUT_EMAIL_SIMPLE): JahiaNode {
  ...
  if (data.autocomplete)
    properties.push({ name: "autocomplete", value: data.autocomplete, type: "STRING" });
  if (data.multiple !== undefined)
    properties.push({ name: "multiple", value: String(data.multiple), type: "BOOLEAN" });
  ...
  if (data.list && Array.isArray(data.list))
    properties.push({ name: "list", values: data.list, language: "en" });
