...
const formatIntegrityErrors = (errors: IntegrityScanError[]) =>
  errors
    .map((error) => {
      const details = [
        error.checkName,
...
const getChecksToRun = (
  ...
  return cy
    ...
    .then((response: IntegrityChecksResponse) => {
      ...
      const availableChecks = checks.map((check) => check.id).filter(Boolean);
      ...
      if (attempt >= maxPollAttempts) {
        throw new Error(
          `No content-integrity checks are available for execution after ${maxPollAttempts} polling attempts.`,
        );
      }

      return cy
        .wait(pollIntervalMs, { log: false })
        .then(() => getChecksToRun(checksToRun, pollIntervalMs, maxPollAttempts, attempt + 1));
    });
};

const getScanState = (id: string): Cypress.Chainable<IntegrityScanInfo> =>
  cy
    .apollo({
    ...
    .then((response: ScanStateResponse) => {
      ...
      if (graphqlErrors) {
        throw new Error(
          `GraphQL error while loading content-integrity scan '${id}':\n${graphqlErrors}`,
        );
      }
...
const waitForScanCompletion = (
  ...
  maxPollAttempts: number,
  attempt = 0,
): Cypress.Chainable<IntegrityScanInfo> =>
  getScanState(id).then((scan) => {
    ...
    return cy
      .wait(pollIntervalMs, { log: false })
      .then(() => waitForScanCompletion(id, pollIntervalMs, maxPollAttempts, attempt + 1));
  });

const getScanResults = (id: string, pageSize: number): Cypress.Chainable<IntegrityScanResults> =>
  cy
    .apollo({
    ...
    .then((response: ScanResultsResponse) => {
      ...
      if (graphqlErrors) {
        throw new Error(
          `GraphQL error while loading content-integrity results '${id}':\n${graphqlErrors}`,
        );
      }
...
export const runContentIntegrityScan = ({
  ...
  return getChecksToRun(checksToRun, pollIntervalMs, maxPollAttempts)
    .then((resolvedChecksToRun) =>
      cy.apollo({
    ...
    .then((response: StartScanResponse) => {
      ...
      if (graphqlErrors) {
        throw new Error(
          `GraphQL error while starting content-integrity scan for '${startNode}':\n${graphqlErrors}`,
        );
      }
...
export const assertContentIntegrityClean = (
  options: AssertContentIntegrityCleanOptions,
): Cypress.Chainable<IntegrityScanResults> =>
