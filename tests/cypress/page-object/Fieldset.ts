...
/** Fieldset component - Groups form elements together Corresponds to fmdb:fieldset node type */
export class Fieldset extends BaseComponent {
  ...
  getTextInput(name: string): TextInput {
    return new TextInput(this.getElements().find(`input[type="text"][name="${name}"]`));
  }

  /** Get an email input by its name attribute */
  getEmailInput(name: string): EmailInput {
    return new EmailInput(this.getElements().find(`input[type="email"][name="${name}"]`));
  }

  /** Get a date input by its name attribute */
  getDateInput(name: string): DateInput {
    return new DateInput(this.getElements().find(`input[type="date"][name="${name}"]`));
  }
  ...
  getColorInput(name: string): ColorInput {
    return new ColorInput(this.getElements().find(`input[type="color"][name="${name}"]`));
  }

  /** Get a single checkbox by its name attribute */
  getCheckbox(name: string): CheckboxInput {
    return new CheckboxInput(this.getElements().find(`input[type="checkbox"][name="${name}"]`));
  }

  /**
   * Get a checkbox group by its name attribute The name is typically the shared name of all
  ...
  getRadio(name: string): RadioInput {
    return new RadioInput(this.getElements().find(`input[type="radio"][name="${name}"]`));
  }

  /** Get a select input by its name attribute */
  getSelectInput(name: string): SelectInput {
    return new SelectInput(this.getElements().find(`select[name="${name}"]`));
  }

  /** Get a textarea by its name attribute */
  getTextarea(name: string): TextareaInput {
    return new TextareaInput(this.getElements().find(`textarea[name="${name}"]`));
  }

  /** Get a file input by its name attribute */
  getFileInput(name: string): FileInput {
    return new FileInput(this.getElements().find(`input[type="file"][name="${name}"]`));
  }
  ...
  getButtonInput(identifier: string): ButtonInput {
    return new ButtonInput(
      this.getElements()
        .find(`button:contains("${identifier}"), button[name="${identifier}"]`)
        .first(),
    );
  }
  ...
  getHiddenInput(name: string): HiddenInput {
    return new HiddenInput(this.getElements().find(`input[type="hidden"][name="${name}"]`));
  }
