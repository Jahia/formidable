...
/** File input form element Handles file upload and validation */
export class FileInput extends FormElement {
  ...
  /**
   * Attach file(s) and retry until the rendered selection reaches the expected count. Use this only
   * for scenarios where the client-side file list is known to be flaky or delayed.
   */
  attachFileAndWaitForCount(
    filePath: string | string[],
    expectedCount: number,
    maxAttempts = 3,
  ): this {
