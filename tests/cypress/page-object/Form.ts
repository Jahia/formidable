...
/** Form component - Main form container Corresponds to fmdb:form node type */
export class Form extends BaseComponent {
  ...
  getDateTimeLocalInput(name: string): DateTimeLocalInput {
    return new DateTimeLocalInput(this.findByName('input[type="datetime-local"]', name));
  }

  /** Get a color input directly in the form by its name attribute */
  getColorInput(name: string): ColorInput {
    return new ColorInput(this.findByName('input[type="color"]', name));
  }

  /** Get a single checkbox directly in the form by its name attribute */
  getCheckbox(name: string): CheckboxInput {
    return new CheckboxInput(this.findByName('input[type="checkbox"]', name));
  }
  ...
  getHiddenInput(name: string): HiddenInput {
    return new HiddenInput(this.findByName('input[type="hidden"]', name));
  }
  ...
  getNextButton(): ButtonInput {
    return new ButtonInput(this.get().find(".fmdb-form-actions .fmdb-next-btn").first());
  }

  getPreviousButton(): ButtonInput {
    return new ButtonInput(this.get().find(".fmdb-form-actions .fmdb-prev-btn").first());
  }

  getNewFormButton(): ButtonInput {
    return new ButtonInput(this.getPageRoot().find(".fmdb-message .fmdb-new-form-btn").first());
  }
  ...
  getSubmitButton(): ButtonInput {
    return new ButtonInput(this.get().find('.fmdb-form-actions button[type="submit"]').first());
  }

  /** Get the reset button */
  getResetButton(): ButtonInput {
    return new ButtonInput(this.get().find('.fmdb-form-actions button[type="reset"]').first());
  }
