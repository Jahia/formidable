...
by `formidable-elements`: the stable class hooks, the CSS variables, and the edit-mode cues — and,
in a section of its own, those of the optional `formidable-extended-inputs` module.
...
Its structure, for the rare rule that must reach inside it — the variables below are the intended
surface:
...
the shared hooks above (`fmdb-form-group` on its wrapper, `fmdb-group-legend`,
