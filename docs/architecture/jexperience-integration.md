...
| `fmdbmix:jExperiencePrefill` | jexperience-engine, CND | The author's "fill this field from the visitor profile when the page opens": a mixin that `extends` the marker, so that jcontent gives it a fieldset with a switch in the same section; its one option, `jExperiencePrefillThen`, says what the page does once the profile's value is in the field — leave it editable, make it read-only, or hide it, the value still submitted. A field is prefilled when it carries this mixin, is mapped, and is not sensitive; the profile's value replaces a default value the author gave the field, and a field the profile has no value for keeps its default, untouched by the option. |
...
native attribute on a text-like control; a select, a radio or a checkbox has none, and `disabled` would take
the value out of the submission, so they get `aria-readonly` and a listener that puts the prefilled state
back on every change, before it bubbles to the logic — the base stylesheet tones the field down and takes
the pointer off those controls (`prefill.css`, two variables). Hidden sets `display: none` on the field's
wrapper, the value still submitted; the range slider, told through the event, disables itself and keeps its
mirror. Both write `data-fmdb-prefilled` (`readonly` or `hidden`) on the wrapper, the styling hook. Neither
touches a field the prefill left alone — the visitor has to be able to fill it — and a value the field's own
validation rejects is never hidden, since the visitor could neither see the error nor fix it. The conditional
logic keeps such a wrapper out of sight when a rule shows it again, while still enabling its controls and
reading it as a source: it is not hidden *by the logic*.
...
// page opens"; the profile's value replaces the author's default, a missing value leaves the field alone;
// the one option inside says what the page does with the field once the value is in: editable, read-only, hidden.
...
switch is the author's "prefill this field", and its one option — what the page does once the profile's value
