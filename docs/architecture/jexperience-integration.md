...
| 4 | Prefill of every mapped shape from the context's profile properties, empty fields only unless the author lets a default give way; `formidable:ready` on the island | **Shipped 2026-09-17 (PR #336)**: on the local instance, a visit with a known visitor's `context-profile-id` cookie fills the simple form's three fields and the complete form's date island and radio once `wemLoaded` and `formidable:ready` are there, one `digitalDataOverrides` push for the page; a visitor jCustomer does not know gets empty fields; the HTML is the same for both. The `formidable:ready` contract is asserted in Cypress (spec 49) |
