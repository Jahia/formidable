...
| 2026-09-17 | **The removal of the snapshot's prefill property is refused by the definitions check**, said in the upgrade notes (HDU's review) | Measured: with the earlier CND registered, `installOrUpgradeBundle` answers `install: []` and HTTP 200, and only the log says `Major change in definition … REMOVED, cancel module deployment`. Nothing released carries the property, so no migration; the note says what to untick and what to switch on again |
