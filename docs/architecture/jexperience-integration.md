...
| `fmdbmix:jExperiencePrefill` | jexperience-engine, CND | The author's "fill this field from the visitor profile when the page opens": a mixin that `extends` the marker, so that jcontent gives it a fieldset with a switch in the same section, and nothing inside: the switch is the whole decision. A field is prefilled when it carries this mixin, is mapped, and is not sensitive; the profile's value replaces a default value the author gave the field, and a field the profile has no value for keeps its default. |
...
| `formidable:prefill` | jexperience-engine → formidable-elements, `Range.client.tsx` | DOM `CustomEvent` (bubbling) the client script dispatches on a **hidden** control after writing it, with `{value}`. The range slider's named control is a hidden mirror of the island's state, which a write cannot reach — React reconciles it back at the next render — so the island takes the value from the event, under the prefill's own rules spelt in `rangePrefill.ts` (visitor's answer kept, author's default given way to, a number within the bounds snapped to the step). A plain hidden field has no island and keeps the written value. |
...
profile property it reads. **What the form
...
<script type="application/json" data-formidable-jxp="FORM-UUID">
  {"formId": "FORM-UUID", "name": "Contact form", "path": "/sites/mysite/contents/contact",
   "prefill": {"firstName": {"property": "firstName"}, "email": {"property": "email"}}}
</script>
```
...
switch is the author's "prefill this field", and it holds nothing else: the profile's value replaces a default
the author gave the field, a missing value leaves it (the "replace the field's default value" option it held
for a day went with that decision, see the decision log of 2026-09-18).
...
| 4 | Prefill of every mapped shape from the context's profile properties, the profile's value replacing an author's default (since 2026-09-18; empty fields only at first); `formidable:ready` on the island | **Shipped 2026-09-17 (PR #336)**: on the local instance, a visit with a known visitor's `context-profile-id` cookie fills the simple form's four fields — the phone behind its logic included — and, on the complete form, the date island (the visitor's calendar day), the gender radio, the country select and the kids number, whose author default of 1 gives way to the profile's value; a field the visitor typed in and cleared before the context landed stays empty; all of it once `wemLoaded` and `formidable:ready` are there, one `digitalDataOverrides` push for the page; a visitor jCustomer does not know gets empty fields; the HTML is the same for both. The `formidable:ready` contract is asserted in Cypress (spec 49), the range island's prefill rules in vitest |
