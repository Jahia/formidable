...
/**
 * Container-level view that renders child nodes through {@link LogicAwareRender}, providing shared
 * conditional-logic hiding for fieldsets, steps, and field lists.
 *
...
jahiaComponent(
	...
	(_props, { currentNode, currentResource }) => {
		const elementNodes = (Array.from(currentNode.getNodes()) as FormContainerNode[]).filter(
			(node) => !node.isNodeType("fmdb:logicList"),
		);
		...
		const preferCompactStepView =
			currentResource.getModuleParams().get("preferCompactStepView")?.toString() === "true";
		const hideStepsAfterFirst =
			currentResource.getModuleParams().get("hideStepsAfterFirst")?.toString() === "true";
		...
		const children = elementNodes.map((elementNode) => {
			...
			const childParameters =
				isStep && hideStepsAfterFirst && currentStepIndex > 0
