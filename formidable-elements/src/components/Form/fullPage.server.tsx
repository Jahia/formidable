 ...
 * declares its own fmdb:form template (priority above 1) and renders this view. On that site
 * the template set's template also serves the Page Builder, so the nested render is read-only
 * like hidden.pageBuilder's: one module for the form node, not two nested boxes for the same
 * path (the jContent #2627 symptom).
 */
jahiaComponent(
	{
		componentType: "view",
