...
export default function Form({
	...
	const isSubmitBlocked =
		isLoading || isSubmitDisabled || (!!captcha && (!isMultiStep || isLastStep) && !isCaptchaValid);
	...
	return (
		<>
			...
			{hasMessage && !isLoading && (
				<div
					className={clsx(`fmdb-message fmdb-message-${messageType}`, classes.message)}
					role="alert"
				>
				...
				{isMultiStep && showStepsNav && (
					<nav className={clsx("fmdb-steps-nav", classes.stepsNav)} aria-label={t("stepsNav")}>
						{visibleStepIndices.map((stepIdx, visibleIdx) => (
							...
							>
								<span className={clsx("fmdb-step-number", classes.stepNumber)}>
									{visibleIdx + 1}
								</span>
