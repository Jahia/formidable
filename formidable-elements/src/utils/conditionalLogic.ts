export type SupportedConditionalSourceType =
	| "fmdb:select"
	| "fmdb:radio"
	| "fmdb:checkbox"
	| "fmdb:inputDate";
...
const getSourceFieldState = (wrapper: HTMLElement): SourceFieldState => {
	...
	if (select) {
		if (select.multiple) {
			return {
				values: Array.from(select.selectedOptions)
	...
	if (checkboxInputs.length > 0) {
		if (checkboxInputs.length === 1) {
			return {
				values:
					checkboxInputs[0].checked && checkboxInputs[0].value ? [checkboxInputs[0].value] : [],
				checked: checkboxInputs[0].checked,
			};
		}

		return {
			values: checkboxInputs
...
export const applyConditionalLogicVisibility = (form: HTMLFormElement) => {
	...
	const wrappersByNodeId = new Map(
		wrappers.filter((w) => w.dataset.fmdbNodeId).map((w) => [w.dataset.fmdbNodeId!, w]),
	);
	const wrappersByName = new Map(
		wrappers.filter((w) => w.dataset.fmdbNodeName).map((w) => [w.dataset.fmdbNodeName!, w]),
	);
