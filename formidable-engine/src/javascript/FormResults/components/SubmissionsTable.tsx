...
export const SubmissionsTable = ({
  ...
  useEffect(() => {
    ...
    const updatedSelection = submissions.find(
      (submission) => submission.uuid === selectedSubmission.uuid,
    );
  ...
  useEffect(() => {
    ...
    const row = tableRef.current.querySelector<HTMLElement>(
      `[data-submission-uuid="${selectedSubmission.uuid}"]`,
    );
    if (row) {
    ...
    return (
      <div style={{ textAlign: "center", padding: "48px" }}>
        <Typography variant="heading" weight="bold">
          {t("formResults.empty.noSubmissions")}
        </Typography>
      </div>
    );
  }
  ...
  return (
    <Paper
      hasPadding={false}
      style={{ display: "flex", flexDirection: "column", height: "100%", borderRadius: "0" }}
    >
      <div
        ref={tableRef}
        tabIndex={0}
        ...
        >
          <TableHead>
            <TableRow>
              ...
              <TableHeadCell width="80px" textAlign="center">
                {t("formResults.table.files")}
              </TableHeadCell>
              <TableHeadCell width="90px" textAlign="center">
                {t("formResults.table.filledFields")}
              </TableHeadCell>
          ...
          <TableBody>
            {submissions.map((submission) => (
              ...
              >
                ...
                <TableBodyCell width="90px">{submission.locale ?? ""}</TableBodyCell>
