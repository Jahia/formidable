...
export const parseRule = (value?: string): ConditionalLogicRule => {
  ...
  try {
    ...
    const sourceFieldType = SUPPORTED_SOURCE_TYPES.includes(
...
export const extractLanguage = (props: SelectorProps): string => {
  const editorContext = extractEditorContext(props);
  const fromWindow = (window as unknown as { contextJsParameters?: { uilang?: string } })
    .contextJsParameters?.uilang;

  return (
    editorContext?.nodeData?.language ??
...
const mapSourceField = (node: GraphNode): SourceFieldOption | null => {
  ...
  const choiceValues =
    type === "fmdb:inputDate" ? [] : parseJsonArrayValue(choiceProperty?.values ?? []);
...
export const buildSourceFieldOptions = (
...
export const buildLogicIdToSourceMap = (
  logicSrcNodes: LogicSrcNode[] = [],
