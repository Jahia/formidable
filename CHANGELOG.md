...
* Added a form actions zone to the Page Builder: see, order and add a form's actions from the page it sits on (#294)
...
* Added a published package so a module of your own renders form fields like the built-in ones, masks included (#317)

  Add `@jahia/formidable-library` to your module (`yarn add @jahia/formidable-library`) to import the help block, the validation-message attributes and the input mask instead of copying their markup.

* Added automatic jExperience mapping rules: a published form with mapped fields feeds the visitor profile (#325)

  Map a field to a visitor profile property in the form editor and publish the form: jExperience gets the rule that writes each submission into the profile, updated at every publication and removed when the form is unpublished or deleted. Nothing to configure in jExperience.
...
* Added a prefill that comes back when a visitor resets a form or starts another one (#338)

* Added a per-field choice of what follows a prefill: leave the field editable, make it read-only or hide it (#338)

* Added field actions: your own check of a field's value, run while the visitor types and again at submission (#344)
