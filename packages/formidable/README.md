...
`"@jahia/formidable": "workspace:*"`: Yarn links the sources (`exports` → `src/index.ts`), every
...
forces the registry — `"@jahia/formidable": "npm:^0.5.0"`. A module outside this repository has no
such workspace: `yarn add @jahia/formidable` simply goes to npm.
